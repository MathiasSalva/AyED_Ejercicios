...
int main()
{
    ...
    while (x=pop(pila))
        cout << "pop: " << x << endl;
    push(pila,0);
    //Qué pasaría si quisiérmos incluir el cero como valor posible para elementos de la
    //pila. Podemos agregar un parámetro bool que indique si la pila está vacía o no,
    //lo resolvemos en pop_v2.
    x=pop_v2(pila,error);
    if(!error)
